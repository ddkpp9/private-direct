...
// Based on egerndaddy/quick-start.
// Private-key compatibility fixes maintained by ddkpp9.
//
// Required env: host, username, password (or privateKey), port (default 22)
// Optional env: passphrase (for encrypted private keys)
...
export default async function (ctx) {
  ...
  try {
    ...
    const username = String(env.username || '').trim() || 'root';
    const password = String(env.password || '');
    const passphrase = String(env.passphrase || '');

    const portText = String(env.port || '22').trim();
    const port = Number.parseInt(portText, 10);
    if (!Number.isInteger(port) || port < 1 || port > 65535) {
      throw new Error('SSH 端口必须是 1–65535 之间的整数');
    }

    let privateKey = String(env.privateKey || '').trim();
    ...
    // Egern module fields may flatten a pasted multiline key into one line.
    // Rebuild the BEGIN/body/END layout for PEM and OpenSSH private keys.
    if (privateKey && !privateKey.includes('\n')) {
      const flattened = privateKey.match(
        /^(-----BEGIN ([A-Z0-9 ]*PRIVATE KEY)-----)\s*([\s\S]*?)\s*(-----END \2-----)$/
      );
      if (flattened) {
        const body = flattened[3].replace(/\s+/g, '');
        privateKey = `${flattened[1]}\n${body}\n${flattened[4]}`;
      }
    }

    if (!host) throw new Error('请填写 SSH 主机地址');
    if (!privateKey && !password) {
      throw new Error('请填写 SSH 密码或完整私钥内容');
    }
    if (privateKey && !/^-----BEGIN [A-Z0-9 ]*PRIVATE KEY-----\n/.test(privateKey)) {
      throw new Error('私钥格式无效：请粘贴完整私钥内容，不要填写 .pub 公钥或文件路径');
    }
    ...
    if (session) {
      try {
        ...
        // Ignore close errors so the original result remains visible.
      }
    }
  }
